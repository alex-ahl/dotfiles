...
agent_launch_cmd() {
  ...
  if [ -n "$2" ]; then printf 'sv shell "$PWD" -- zsh -lc "%sclaude \\"/handoff-resume %s\\""' "$(_agent_env "$1")" "$2"
  else                 printf 'sv shell "$PWD" -- zsh -lc "%sclaude"' "$(_agent_env "$1")"; fi
}
agent_continue_cmd() { printf 'sv shell "$PWD" -- zsh -lc "%sclaude --continue"' "$(_agent_env "$1")"; }
