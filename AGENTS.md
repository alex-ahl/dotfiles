...
- **Nothing here is live for the sandbox.** The sandbox account cannot read
  ...
  `/Users/Shared/sv-$USER/agent-runtime/`, rebuilt by `scripts/deploy-runtime.sh`
  (which `./install.sh` also calls). Edit or add a skill or a shared script and a
  sandboxed pane keeps running the old one until then — if a change appears to
