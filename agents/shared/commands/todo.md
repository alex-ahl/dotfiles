...
- **Tick** — the argument names something finished, or is empty and this session clearly just
  finished something.
- **List** — the argument is `list`, `show`, or `ls`. Print open items from **both** brains,
  work first, then stop.
...
2. One line, `- [ ] <item>`, no trailing period.
...
3. Flip `[ ]` to `[x]` in place and append ` — done YYYY-MM-DD`. Don't move the line.
